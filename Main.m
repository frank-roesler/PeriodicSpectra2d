...
resolution_z = 4;               % resolution in complex plane
resolution = 10;                % theta resolution
                                % Size of matrix K
...
%     Start gradient descent:
    ...
    while max(abs(endpoints))==0
        for n=1:iters
            z_start = starting_points(n);
                [z_c,ctr] = newton_method(z_start, stepsize, maxiter, tol, theta_x, theta_y, k, z0, N, A);
            if ctr<maxiter
                endpoints(n) = z_c;
            end
        end
    ...
    if theta_x==0 && m>2
        plot(Spectrum+0.00000000001i,'.','MarkerEdgeColor',[0.3 0.3 1], 'MarkerSize',10);
